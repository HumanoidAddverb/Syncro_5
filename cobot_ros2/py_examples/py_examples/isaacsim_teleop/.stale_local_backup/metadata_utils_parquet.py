    ...
    for col in ("observation.state.joint_positions",
        ...
        if col in new_df.columns:
            new_df[col] = new_df[col].apply(
                lambda x: np.array(x, dtype=np.float32) if x is not None else np.zeros(2, dtype=np.float32)
            )

    if "base.timestamp_us" in new_df.columns:
        new_df["base.timestamp_us"] = new_df["base.timestamp_us"].astype("int64")
