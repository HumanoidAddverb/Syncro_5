...
class SimDataLogger(threading.Thread):
    ...
    def _write_row(self, frame: dict):
        ...
        row = {
            ...
            "timestamp":                     ts,
            "mode":                          "sim_teleop",
            "state.joint_positions":         state_vec.tolist(),   # [7] or [9]
            "state.base":                    [0.0, 0.0],
            "action.robot":                  action_vec.tolist(),  # [7] or [9]
            "action.base":                   [0.0, 0.0],
            "base.timestamp_us":             int(ts * 1_000_000),
        }
        ...
        for cam in self._cam_names:
            ...
            print(
                ...
                f"arm={np.round(action[:6], 3)}  gripper={obs[6]:.4f}"
                f"camera name = {cam}"
            )
